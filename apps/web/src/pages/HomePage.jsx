...
export default function HomePage({ navigate }) {
  ...
  return (
    <div className="fade-in space-y-6 md:space-y-8">
      ...
      <div className="flex flex-col md:flex-row gap-4 w-full">
        ...
        <div className="flex-1 bg-white dark:bg-[#1E293B] rounded-2xl p-6 flex flex-col justify-between shadow-sm dark:shadow-lg dark:shadow-black/20 relative overflow-hidden border border-slate-200 dark:border-[#334155]/50 min-h-[140px]">
          <div className="relative z-10">
            <h1 className="text-[19px] md:text-[21px] lg:text-[23px] font-bold text-slate-900 dark:text-white mb-1 tracking-tight leading-snug">{greeting}</h1>
            <p className="text-[14px] md:text-[15px] text-slate-500 dark:text-[#94A3B8] mb-4 font-medium">{user.level} · {user.subjects.length} Subjects</p>
          </div>
          <div className="relative z-10 flex flex-wrap items-center gap-2">
            <div className={`inline-flex w-fit bg-slate-100 dark:bg-[#0F172A] ${countdownColor} px-3.5 py-1.5 rounded-full text-[13px] font-bold tracking-wide uppercase border border-current/10`}>
              {countdownText}
            </div>
            <div className={`inline-flex items-center text-[13px] font-medium ${streakColor}`}>
              🔥 {streak.current} day streak
            </div>
            <button
              onClick={() => navigate('calendar')}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[13px] font-semibold bg-[#22C55E]/10 text-[#22C55E] border border-[#22C55E]/20 hover:bg-[#22C55E]/20 transition-colors"
            >
        ...
        <div className="flex flex-row md:flex-col gap-4 md:w-[200px] lg:w-[240px] xl:w-[280px] shrink-0">
          <div className="flex-1 md:flex-none bg-white dark:bg-[#1E293B] rounded-2xl p-5 border-l-[4px] border-[#22C55E] shadow-sm flex flex-col justify-center border border-slate-200 dark:border-[#334155]/50 border-l-[#22C55E]">
            <span className="text-[25px] md:text-[27px] lg:text-[29px] font-bold text-slate-900 dark:text-white mb-0.5 tracking-tight" style={{ fontVariantNumeric: 'tabular-nums' }}>
              {user.stats.questionsSolved || 0}
            </span>
            <span className="text-[12px] md:text-[13px] text-slate-500 dark:text-[#94A3B8] font-medium leading-tight">Questions solved</span>
          </div>
          <div className="flex-1 md:flex-none bg-white dark:bg-[#1E293B] rounded-2xl p-5 border-l-[4px] border-[#3B82F6] shadow-sm flex flex-col justify-center border border-slate-200 dark:border-[#334155]/50 border-l-[#3B82F6]">
            ...
            <span className="text-[12px] md:text-[13px] text-slate-500 dark:text-[#94A3B8] font-medium leading-tight">Papers read</span>
          </div>
          <button
          ...
          >
            ...
            <span className="text-[12px] md:text-[13px] text-slate-500 dark:text-[#94A3B8] font-medium leading-tight flex items-center gap-1">
      ...
      {/* Quick Actions */}
      <div>
        <h2 className="text-[16px] md:text-[17px] font-bold text-slate-900 dark:text-white mb-4 tracking-tight">Quick actions</h2>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 md:gap-4">
          ...
          >
            ...
            <span className="text-[14px] md:text-[15px] font-semibold text-slate-800 dark:text-[#F1F5F9]">Ask AI</span>
          </button>
          ...
          {user.subjects.slice(0, 3).map((sub, idx) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-[14px] md:text-[15px] font-semibold text-slate-800 dark:text-[#F1F5F9] truncate w-full text-center">{sub}</span>
              </button>
            );
          })}
        </div>
      ...
      {/* Recent Activity */}
      <div>
        <h2 className="text-[16px] md:text-[17px] font-bold text-slate-900 dark:text-white mb-4 tracking-tight">Recent activity</h2>
              ...
              <div key={`activity-${activity.id}`} className="bg-white dark:bg-[#1E293B] p-4 rounded-2xl border border-slate-200 dark:border-[#334155]/50 hover:border-slate-300 dark:hover:border-[#475569] transition-all flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start mb-1">
                    <h3 className="text-[15px] font-semibold text-slate-900 dark:text-white truncate pr-2">{activity.subject}</h3>
                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-bold uppercase tracking-wide shrink-0 ${
                      activity.type === 'quiz' ? 'bg-[#F97316]/10 text-[#F97316]' :
                      activity.type === 'paper' ? 'bg-[#3B82F6]/10 text-[#3B82F6]' :
                      'bg-[#22C55E]/10 text-[#22C55E]'
                    }`}>
                  ...
                  <p className="text-[13px] text-slate-500 dark:text-[#94A3B8] truncate">{activity.preview || activity.paper || 'Solved'}</p>
                </div>
              </div>
            ))
          ) : (
            <div className="col-span-full py-8 text-center">
              <p className="text-[13px] text-slate-400 dark:text-[#64748B]">No activity yet. Ask the AI your first question! 👇</p>
            </div>
          )}
        </div>
      </div>
