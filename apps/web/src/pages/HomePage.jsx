...
export default function HomePage({ navigate }) {
  ...
  return (
    <div className="fade-in">
      {/* Hero Section */}
      <div className="w-full flex flex-col md:flex-row gap-4 mb-6 md:mb-8">
        <div className="flex-1 bg-white dark:bg-[#1E293B] rounded-2xl p-6 flex flex-col justify-between shadow-sm dark:shadow-lg dark:shadow-black/20 relative overflow-hidden border border-slate-200 dark:border-[#334155]/50">
          <div className="relative z-10">
            <h1 className="text-[18px] md:text-[22px] font-bold text-slate-900 dark:text-white mb-1 tracking-tight">{greeting}</h1>
            <p className="text-[13px] md:text-[14px] text-slate-500 dark:text-[#94A3B8] mb-5 font-medium">{user.level} · {user.subjects.length} Subjects</p>
          </div>
          ...
          <div className="relative z-10 flex flex-col gap-1">
            ...
            <div className={`text-[12px] font-medium mt-1 ${streakColor}`}>
        ...
        <div className="grid grid-cols-2 gap-4 w-full md:w-[320px]">
          <div className="bg-white dark:bg-[#1E293B] rounded-2xl p-5 border-l-[4px] border-[#22C55E] shadow-sm flex flex-col justify-center border-y border-r border-y-slate-200 dark:border-y-[#334155]/50 border-r-slate-200 dark:border-r-[#334155]/50">
            <span className="text-[24px] md:text-[28px] font-bold text-slate-900 dark:text-white mb-1 tracking-tight" style={{ fontVariantNumeric: 'tabular-nums' }}>
              {user.stats.questionsSolved || 0}
            </span>
            <span className="text-[12px] md:text-[13px] text-slate-500 dark:text-[#94A3B8] font-medium">Questions solved</span>
          </div>
          <div className="bg-white dark:bg-[#1E293B] rounded-2xl p-5 border-l-[4px] border-[#3B82F6] shadow-sm flex flex-col justify-center border-y border-r border-y-slate-200 dark:border-y-[#334155]/50 border-r-slate-200 dark:border-r-[#334155]/50">
            <span className="text-[24px] md:text-[28px] font-bold text-slate-900 dark:text-white mb-1 tracking-tight" style={{ fontVariantNumeric: 'tabular-nums' }}>
              {user.stats.papersRead || 0}
            </span>
            <span className="text-[12px] md:text-[13px] text-slate-500 dark:text-[#94A3B8] font-medium">Papers read</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-2 xl:grid-cols-4 gap-3 md:gap-4 mb-8">
        <button
