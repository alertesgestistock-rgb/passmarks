...
export default function QuizPlayScreen({ navigate, viewState }) {
  ...
  const handleNext = () => {
    if (currentIndex < questions.length - 1) {
      ...
      if (user) {
        ...
        // Simple moving average for totalScore
        ...
        updateUser({
          ...
          quizHistory: [
            { date: new Date().toISOString(), subject, score: finalScore, total: questions.length, answers: answersLog },
            ...(user.quizHistory || [])
          ]
        });
  ...
  const getOptionStyle = (option) => {
    ...
    if (letter === currentQ.answer) {
      return "border-[#22C55E] bg-[#14532D]"; 
    }
    if (selectedAnswer === option && letter !== currentQ.answer) {
      return "border-[#EF4444] bg-[#3B0000]"; 
    }
    return "border-[#334155] bg-[#1E293B] opacity-50"; 
  };
    ...
    <div className="max-w-[600px] mx-auto h-[calc(100vh-140px)] flex flex-col">
      ...
      <div className="mb-6 shrink-0">
        <div className="flex justify-between items-end mb-2">
          <h2 className="text-[13px] font-bold text-[#94A3B8] tracking-widest uppercase">{subject} Quiz</h2>
          <span className="text-[13px] font-bold text-white bg-[#334155] px-2.5 py-0.5 rounded-full font-variant-numeric:tabular-nums">
      ...
      <div className="flex-1 overflow-y-auto hide-scrollbar pb-6">
        <div className="bg-[#1E293B] rounded-2xl p-5 md:p-6 mb-6 shadow-sm border border-[#334155]/50">
          <h3 className="text-[16px] md:text-[18px] font-semibold text-[#F1F5F9] leading-relaxed">
            {currentQ.question}
          </h3>
        </div>
          ...
          {currentQ.options.map((option, idx) => {
            ...
            return (
              <button
                ...
                disabled={isAnswered}
                className={cn(
                  "w-full text-left rounded-2xl p-4 border transition-all flex items-center gap-4 group",
              ...
              >
                <div className={cn(
                  ...
                  "bg-[#334155] text-[#94A3B8] group-hover:bg-[#475569]"
                )}>
                  {letter}
                </div>
                <span className="text-[15px] text-[#F1F5F9] font-medium">{option.substring(option.indexOf('.') + 1).trim()}</span>
              </button>
            );
          })}
        </div>
        ...
        {isAnswered && (
          <div className={cn(
            ...
            selectedAnswer.split('.')[0].trim() === currentQ.answer 
              ? "bg-[#14532D]/30 border-[#22C55E]/30" 
              : "bg-[#3B0000]/30 border-[#EF4444]/30"
          )}>
            <div className="flex items-center gap-2 mb-2 font-bold text-[15px]">
              {selectedAnswer.split('.')[0].trim() === currentQ.answer ? (
                <><CheckCircle2 className="text-[#22C55E]" size={20} /> <span className="text-[#86EFAC]">Correct!</span></>
              ) : (
                <><XCircle className="text-[#EF4444]" size={20} /> <span className="text-[#FCA5A5]">Wrong. Answer is {currentQ.answer}</span></>
              )}
            </div>
      ...
      {/* Footer Actions */}
      <div className="shrink-0 pt-4 border-t border-[#334155]">
        {!isAnswered ? (
          <button
            onClick={handleSubmit}
            disabled={!selectedAnswer}
            className="w-full bg-[#F1F5F9] text-[#0F172A] rounded-xl py-4 text-[16px] font-bold disabled:opacity-50 disabled:bg-[#334155] disabled:text-[#94A3B8] transition-all hover:bg-white active:scale-[0.98]"
          >
            Submit Answer
          </button>
        ) : (
