    ...
    <div className="max-w-[600px] mx-auto h-[calc(100vh-140px)] flex flex-col">
      ...
      <div className="mb-6 shrink-0">
        <div className="flex justify-between items-end mb-2">
          <h2 className="text-[14px] font-bold text-slate-400 dark:text-[#94A3B8] tracking-widest uppercase">{subject} Quiz</h2>
          <span className="text-[14px] font-bold text-slate-700 dark:text-white bg-slate-100 dark:bg-[#334155] px-2.5 py-0.5 rounded-full">
      ...
      <div className="flex-1 overflow-y-auto hide-scrollbar pb-6">
        <div className="bg-white dark:bg-[#1E293B] rounded-2xl p-5 md:p-6 mb-6 shadow-sm border border-slate-200 dark:border-[#334155]/50">
          <h3 className="text-[17px] md:text-[19px] font-semibold text-slate-800 dark:text-[#F1F5F9] leading-relaxed">
            {currentQ.question}
          </h3>
        </div>
        ...
        <div className="flex flex-col gap-3">
          {currentQ.options.map((option, idx) => {
            ...
            return (
              ...
              >
                <div className={cn(
                  "w-[32px] h-[32px] rounded-full flex items-center justify-center shrink-0 font-bold text-[15px] transition-colors",
                ...
                <span className="text-[16px] text-slate-800 dark:text-[#F1F5F9] font-medium">{option.substring(option.indexOf('.') + 1).trim()}</span>
              </button>
            );
          })}
        </div>
        ...
        {isAnswered && (
          <div className={cn(
            ...
            <div className="flex items-center gap-2 mb-2 font-bold text-[16px]">
              {selectedAnswer.split('.')[0].trim() === currentQ.answer ? (
            ...
            <p className="text-[15px] text-slate-600 dark:text-[#CBD5E1] leading-relaxed">
      ...
      <div className="shrink-0 pt-4 border-t border-slate-200 dark:border-[#334155]">
        {!isAnswered ? (
          <button
            ...
            className="w-full bg-slate-900 dark:bg-[#F1F5F9] text-white dark:text-[#0F172A] rounded-xl py-4 text-[17px] font-bold disabled:opacity-40 disabled:bg-slate-200 dark:disabled:bg-[#334155] disabled:text-slate-400 dark:disabled:text-[#94A3B8] transition-all hover:bg-slate-800 dark:hover:bg-white active:scale-[0.98]"
          >
            Submit Answer
          </button>
        ) : (
          <button
            onClick={handleNext}
            className="w-full bg-[#22C55E] text-[#052e16] rounded-xl py-4 text-[17px] font-bold flex items-center justify-center gap-2 hover:brightness-110 active:scale-[0.98] transition-all shadow-md"
          >
