      ...
      <div className="fade-in flex flex-col" style={{ height: 'calc(100dvh - 130px)', minHeight: '500px' }}>
        ...
        <div className="flex items-center gap-3 mb-3 shrink-0">
          ...
          <div className="flex-1 min-w-0">
            <h2 className="text-[15px] font-medium text-slate-900 dark:text-[#F1F5F9] truncate">
          ...
          <button
            onClick={handleAskAI}
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-[#3B82F6] text-white text-[13px] font-medium hover:bg-[#2563EB] transition-colors shrink-0"
          >
            <Sparkles size={13} />
            Ask AI
          </button>
      ...
      <div className="fade-in">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div className="min-w-0">
            <h2 className="text-[17px] font-medium text-slate-900 dark:text-[#F1F5F9] truncate">
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 hide-scrollbar mb-5">
          {SOURCES.map(s => (
            <button
              ...
              className={cn(
                'whitespace-nowrap px-3 py-1.5 rounded-full text-[13px] font-medium border transition-all',
        ...
        {loadingPapers ? (
          ...
          <div className="bg-white dark:bg-[#1E293B] rounded-2xl overflow-hidden border border-slate-200 dark:border-[#334155]/50">
            ...
            >
              <span className="text-[12px] font-medium uppercase tracking-wide text-slate-400 dark:text-[#64748B]">Year</span>
              {paperCols.map(pn => (
                <span key={pn} className="text-[12px] font-medium uppercase tracking-wide text-slate-400 dark:text-[#64748B] text-center">
                  P{pn}
                </span>
              ))}
            </div>
            ...
            {years.length === 0 ? (
              <div className="py-14 text-center text-slate-400 dark:text-[#64748B] text-[14px]">
              ...
              years.map((year, idx) => (
                <div
                  ...
                  style={{ gridTemplateColumns: `1fr repeat(${paperCols.length}, 1fr)` }}
                >
                  <span className="text-[14px] font-medium text-slate-700 dark:text-[#F1F5F9]">{year}</span>
                  {paperCols.map(pn => {
                      ...
                      <div key={pn} className="flex justify-center">
                        {url ? (
                          <button
                            ...
                            className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-green-100 dark:bg-[#14532D] text-green-700 dark:text-[#22C55E] text-[12px] font-medium hover:bg-green-200 dark:hover:bg-[#166534] transition-colors"
                          >
                            <FileText size={11} />
                            Open
                          </button>
                        ) : (
                          <span className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-[#0F172A] text-slate-400 dark:text-[#475569] text-[12px]">
                            <Lock size={10} />
                            Soon
                          </span>
                        )}
    ...
    <div className="fade-in">
      ...
      <div className="flex gap-2 mb-4">
        {[{ key: 'A_Level', label: 'A Level' }, { key: 'O_Level', label: 'O Level' }].map(lv => (
          <button
            ...
            className={cn(
              'flex-1 py-2.5 rounded-xl text-[15px] font-medium border transition-all',
      ...
      <div className="relative mb-3">
        ...
        <input
          ...
          className="w-full bg-white dark:bg-[#1E293B] text-slate-900 dark:text-[#F1F5F9] rounded-2xl py-3.5 pl-11 pr-4 text-[15px] outline-none border border-slate-200 dark:border-[#334155]/50 focus:border-[#22C55E] transition-all placeholder:text-slate-400 dark:placeholder:text-[#64748B]"
        />
      </div>
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 hide-scrollbar mb-5">
        {CATEGORY_FILTERS.map(cat => (
          <button
            ...
            className={cn(
              'whitespace-nowrap px-4 py-2 rounded-full text-[14px] font-medium border transition-all h-[36px]',
        ...
        {activeCategories.length > 0 && (
          <button
            ...
            className="whitespace-nowrap px-3 py-2 rounded-full text-[13px] text-red-400 border border-transparent hover:border-red-200 transition-all h-[36px]"
          >
            Clear
          </button>
        )}
      ...
      {/* Error */}
      {error && (
        <div className="mb-4 p-3 rounded-xl bg-red-50 dark:bg-red-900/20 text-red-500 text-[14px]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {filteredSubjects.map(sub => (
            ...
            >
              <div className="w-[42px] h-[42px] rounded-xl bg-slate-100 dark:bg-[#0F172A] flex items-center justify-center shrink-0">
                <span className="font-medium text-[16px] text-slate-700 dark:text-[#F1F5F9]">
                  {sub.subject.charAt(0)}
                </span>
              </div>
              <div className="flex-1 min-w-0">
                <h3 className="text-[15px] font-medium text-slate-900 dark:text-[#F1F5F9] truncate">{sub.subject}</h3>
                <p className="text-[13px] text-slate-400 dark:text-[#64748B]">{sub.subject_code} · {sub.category}</p>
              </div>
          ...
          {!loadingSubjects && filteredSubjects.length === 0 && !error && (
            <div className="col-span-full py-12 text-center">
              <p className="text-slate-500 dark:text-[#94A3B8] text-[14px]">No subjects found</p>
              <p className="text-slate-400 dark:text-[#64748B] text-[13px] mt-1">Try 'Physics' or '0780'</p>
            </div>
          )}
        </div>
      )}
